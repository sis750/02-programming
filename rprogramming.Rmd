...
Don't forget about outputting, or saving, the data too. Write it into your preferred format. I recommend saving in `.RData` format for yourself and in `.csv` format if you intend to distribute the data for public use.[^2]
...
## Pipes
Pipes are awesome and *totally* intuitive... once you understand them. The base R pipe, `|>` and `magrittr` pipe, `%>%`, push the output from one code segment forward as the first input for another code segment. It saves you from creating and tracking loads of intermediate objects. Compare:
